...
parser.add_argument('--caps_dim', type=int, default=16)
...
parser.add_argument('--name_model', type=str, default='Attention', choices=["Attention", "Capsule"])
...
def evaluate(model, datamanager, data_, name=None):
    ...
    while st < len(data_):
        ...
        batched_data = datamanager.gen_batched_data(selected_data, flag_label_respresentation=0)
        outputs, pred_, _ = model.stepTrain(batched_data, inference=True)
...
def visualize(model, datamanager, data_):
    loss = np.zeros((3, ))
    st, ed, times = 0, FLAGS.batch_size, 0
    fs = [open(os.path.join(FLAGS.save_dir, '{}.txt'.format(i)),'w') for i in range(8)]
    while st < len(data_):
        selected_data = data_[st:ed]
        batched_data = datamanager.gen_batched_data(selected_data, flag_label_respresentation=0)
        outputs, pred_, logits = model.stepTrain(batched_data, inference=True)
        st, ed = ed, ed+FLAGS.batch_size
        logits_middle = logits['middle'].cpu().detach().numpy()
        logits_output = logits['output'].cpu().detach().numpy()
        for i in range(FLAGS.batch_size):
            if pred_.shape[0] < FLAGS.batch_size:
                break
            prob = pred_[i]
            lb = batched_data['labels'][i]
            sent = batched_data['sentence'][i]
            predict = np.argmax(prob)
            flag = "[{}-{}] ".format(lb, predict)
            for j in range(8):
                fs[j].write(flag + ' '.join(sent) + '\n')
                fs[j].write(' '.join([str(logits_middle[i, k, j]) for k in range(len(sent))]) + '\n')
            # else:
            #     print("False: " + ' '.join(sent))
            #     print(' '.join([str(logits[i, k, lb]) for k in range(len(sent))]))
...
if __name__ == "__main__":
    ...
    #             if name == 'dev' and acc > max_acc:
    #                 max_acc = acc
    #                 model.save_model("%s" %(FLAGS.save_dir), FLAGS.name_model)
    ...
    model = torch.load("{}/modelCapsule.pkl".format(FLAGS.save_dir))
    #model.model_name = 'Capsule'
    visualize(model, datamanager, data['test'])
